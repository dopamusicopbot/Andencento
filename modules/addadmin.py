...
@Client.on_message(filters.command(['refreshadmins', 'refreshadmins@Andencentovcbot']) & ~filters.edited & ~filters.bot)
@database_check
@chat_allowed
@admin_mode_check
...
@Client.on_message(filters.command(['auth', 'auth@Andencentovcbot']) & ~filters.edited & ~filters.bot)
@database_check
@chat_allowed
@admin_check
...
@Client.on_message(filters.command(['unauth', 'unauth@Andencentovcbot']) & ~filters.edited & ~filters.bot)
@database_check
@chat_allowed
@admin_check
...
@Client.on_message(filters.command(['listadmins', 'listadmins@Andencentovcbot']) & ~filters.edited & ~filters.bot)
@database_check
@chat_allowed
@admin_check
...
@Client.on_message(filters.command(['adminmode', 'adminmode@Andencentovcbot']) & ~filters.edited & ~filters.bot)
@database_check
@chat_allowed
@admin_check
        ...
        if len(message.command) > 1 and message.command[1].lower() in ['on', 'off']:
            ...
            if current_client.get('admin_mode') == new_status:
                ...
                if new_status is True:
                    msg = msg + f"\n\n__Now only admins will be only allowed to perform bot actions.__"
                else:
